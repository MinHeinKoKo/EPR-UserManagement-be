...
@section('content')
    <div class="w-full h-screen p-8">
        <form action="{{ route('roles.update', $role->id) }}" method="post" class="border border-blue-300 p-5 w-[400px] rounded flex flex-col gap-4 items-center" id="roleForm">
            @csrf
            @method('PUT')
            <input type="text" name="name" value="{{ $role->name  }}" class="w-full px-6 py-1 border rounded-md border-primary" placeholder="Enter Role's name" required id="name"/>
            @error('name')
